...
def base(**overrides):
    ...
    kw = dict(
        ...
        # Genuine top-level repro: the operator messaged 'rhea' directly; rhea
        # consulted mini and is now relaying the answer. rhea IS the chain root,
        # so root agent id == the agent running this terminator turn.
        this_agent_id="rhea",
        chain_root_agent_id="rhea",
    )
    kw.update(overrides)
    return kw
...
def test_operator_rooted_real_channel_posts():
    ...
    # The exact repro from log.txt: rhea relays mini's answer to the operator.
...
def test_nested_internal_peer_stays_silent():
    ...
    check("internal transport stays silent",
          _terminator_text_surfaces(**base(
              ...
              channel_conversation_id="internal:peer-rhea")) is False)
