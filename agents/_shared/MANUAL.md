...
├── REMINDER.md           # (optional, legacy) Plain system file — loads ONLY if listed in system_files (rhea and miku do). The per-turn uncached end-of-payload injection mechanic was REMOVED; no special handling remains.
...
- **`restart_flask_app(reason: str = "")`** — restart the separate
  flipflaskapp systemd user unit (`systemctl --user`). Owner-only —
...
- **Key absent / `null`** → no narrowing. The token gets the full `auth.external`
  ...
  existing miniflip game token — which has no `allowed_tools` key — is
  unaffected.)
...
on miniflip; not on other agents.
...
- Retry-heuristic location — the four in-loop turn-retry heuristics
  ...
  `agents/rhea/audits/full_audit_2026-06-07.md`). The decision
