...
Lightweight observability layer for things like the FlipFlaskApp openflip tab.
...
def on_turn_start(agent_id: str, channel, *, talking_with: str = "") -> None:
    ...
    The FlipFlaskApp openflip tab uses this to render inter-agent chain
    lines on the canvas.
    """
...
def on_turn_end(agent_id: str) -> None:
    ...
    state["talking_with"] = ""
    _write(agent_id, state)
    # Turn boundary: now is when external observers (e.g. the FlipFlaskApp
