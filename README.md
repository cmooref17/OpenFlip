...
**Don't edit git-tracked framework files to add your own customizations.**
OpenFlip is a live repo — a `git pull` overwrites tracked files, so any changes
you wedge into them won't survive an update. Put your additions in the gitignored
extension spots instead (config, your agent dir, local tool/transport files),
where updates can't touch them. (This applies to AI coding assistants too — they
default to editing core files; redirect them.)

Step-by-step recipes for each are in
**[agents/_shared/MANUAL.md](agents/_shared/MANUAL.md)** §14.
